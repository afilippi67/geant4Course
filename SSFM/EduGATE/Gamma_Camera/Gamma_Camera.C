...
public:
  FileList(const TGWindow *p, const TGWindow *GCmain, UInt_t w, UInt_t h);
	virtual ~FileList();
...

{
  ...
  globalPosZ_Y_Ewin = new TH2F("globalPosZ_Ewin","globalPosY_Ewin",600,-150.,+150.0,400,-100.0,+100.0);
...
void MyMainFrame::GammaCamera(TString fname)
{
	...
	Float_t			scatter_phantom=0, scatter_null=0, scatter_compartment=0, scatter_table=0;
	Float_t			scatter_collim=0, scatter_shielding=0, primary_event=0, scatter_crystal=0;
	Float_t			order1=0, order2=0, order3=0, order4=0,
	  ordersup=0, ordertot=0;
...
//	Float_t CE_value = 0.141;
//	Float_t CE_value = 0.364;
//	Float_t CE_value = 0.90;
//	Float_t	LETH = CE_value - 0.1*CE_value;
//	Float_t UETH = CE_value + 0.1*CE_value;
	
	Float_t CE_value = 0.20;
	Float_t	LETH = CE_value - 0.1*CE_value;
	Float_t UETH = CE_value + 0.1*CE_value;	
	
	cout << endl;
	cout << " Energy setting (central, leth, ueth):  " << CE_value << " " << LETH << " " <<  UETH << endl;
	cout << endl;
...
FileList::FileList(const TGWindow *p, const TGWindow *GCmain, UInt_t w, UInt_t h)
{
   // Create transient frame containing a filelist widget.

   TGLayoutHints *lo;	

   fMain = new TGTransientFrame(p, GCmain, w, h);
...
void Gamma_Camera()
{
   ...
   GCmain = new MyMainFrame(gClient->GetRoot(), 350, 80);
}
