...
public:
	TestFileList(const TGWindow *p, const TGWindow *CCmain, UInt_t w, UInt_t h);
...
void MyMainFrame::ChooseFile()
{
  ...
  new TestFileList(gClient->GetRoot(), fMain,  500, 500);
...
TestFileList::TestFileList(const TGWindow *p, const TGWindow *CCmain, UInt_t w, UInt_t h)
{
   // Create transient frame containing a filelist widget.

   TGLayoutHints *lo;	

   fMain = new TGTransientFrame(p, CCmain, w, h);
...
void Coin_Chan()
{
   ...
   CCmain = new MyMainFrame(gClient->GetRoot(), 350, 80);
}
